 ...
 *   depth     (max 25) — volume of content created / collected
 *   breadth   (max 20) — number of distinct platforms connected
 *   standing  (max 20) — social proof: followers, stars, playtime, engagement
 */

import type { IdentityData } from "@/lib/vana-sources";

export interface ScoreComponent {
  key: "age" | "depth" | "breadth" | "standing";
  label: string;
  points: number;
  max: number;
  detail: string;
...
function tipsFor(result: { components: ScoreComponent[]; count: number; connected: string[] }): string[] {
  ...
  if (byKey.breadth && result.count < 3) {
    tips.push(`Connect ${3 - result.count} more platform${result.count < 2 ? "s" : ""} for a breadth bonus.`);
  }
...
export function computeSoulScore(identities: IdentityData[]): SoulScoreResult {
  ...
  // Age — max 35. Best single account age, capped at 10+ years.
  const maxAge = profiles.reduce((m, p) => Math.max(m, p.ageYears), 0);
  const agePoints = Math.round(clamp(maxAge / 10, 0, 1) * 35);
  const ageDetail =
    maxAge > 0
      ? `${maxAge.toFixed(1)} yr old account${profiles.length > 1 ? " (oldest)" : ""}`
      : "Nothing connected yet that carries a date.";

  // Depth — max 25. Log scale: 10 items → ~10pts, 100 → ~18, 1000+ → 25.
  const totalDepth = profiles.reduce((s, p) => s + p.depth, 0);
  const depthPoints = Math.round(clamp(Math.log10(totalDepth + 1) / 3, 0, 1) * 25);
  const depthDetail =
    totalDepth > 0
      ? `${Math.round(totalDepth)} items across ${count} source${count > 1 ? "s" : ""}`
      : "Counts the posts, videos, repos and tracks you have made.";

  // Breadth — max 20. 1 src → 5, 2 → 10, 3 → 15, 4+ → 20.
  const breadthPoints = clamp(count * 5, 0, 20);
  const breadthDetail =
    count > 0 ? `${count} platform${count > 1 ? "s" : ""} connected` : "Connect multiple platforms to map more of yourself.";

  // Standing — max 20. Log scale on max social proof metric.
  const maxStanding = profiles.reduce((m, p) => Math.max(m, p.standing), 0);
  const standingPoints = Math.round(clamp(Math.log10(maxStanding + 1) / 4, 0, 1) * 20);
  const standingDetail =
    maxStanding > 0
      ? `Best social proof: ${Math.round(maxStanding)} followers/subscribers`
      : "Other people and platforms vouch for you — followers, stars, playtime.";

  const components: ScoreComponent[] = [
    { key: "age", label: "Age", points: agePoints, max: 35, detail: ageDetail },
    { key: "depth", label: "Depth", points: depthPoints, max: 25, detail: depthDetail },
    { key: "breadth", label: "Breadth", points: breadthPoints, max: 20, detail: breadthDetail },
    { key: "standing", label: "Standing", points: standingPoints, max: 20, detail: standingDetail },
  ];
